...
VL_EXPORT
VlSiftFilt *
vl_sift_new_64 (int width, int height,
...
VL_EXPORT
void
vl_sift_calc_raw_descriptor_64 (VlSiftFilt const *f,
...
VL_EXPORT
void
vl_sift_calc_keypoint_descriptor_64 (VlSiftFilt *f,
                                  vl_sift_pix *descr,
...
VL_EXPORT
int
vl_sift_calc_keypoint_orientation (VlSiftFilt *f,
                                    double angles [4],
                                    VlSiftKeypoint const *k)
{
  ...
  /* skip if the keypoint octave is not current */
    if(k->o != f->o_cur){
    //    printf("warning: keypoint octave is not current!\n");
    return 0 ;
    }
...
#define at(dx,dy) (*(pt + xo * (dx) + yo * (dy)))
  ...
  /* only use the maxBin */
  double h0 = hist [maxBin] ;
  double hm = hist [(maxBin - 1 + nbins) % nbins] ;
  double hp = hist [(maxBin + 1 + nbins) % nbins] ;
  
  double di = - 0.5 * (hp - hm) / (hp + hm - 2 * h0) ;
  double th = 2 * VL_PI * (maxBin + di + 0.5) / nbins ;
  angles [ nangles++ ] = th ;
  assert(nangles == 1);
  return nangles;
